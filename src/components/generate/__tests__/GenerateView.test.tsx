...
vi.mock("../SourceTextInput", () => ({
  SourceTextInput: ({
    ...
    <div data-testid="source-text-input">
      <input data-testid="source-text" value={sourceText} onChange={(e) => onSourceTextChange(e.target.value)} />
      <button data-testid="generate-button" onClick={() => onSubmit(sourceText)} disabled={isLoading || !isValid}>
        Generate
      </button>
    </div>
  ),
...
vi.mock("../CandidateList", () => ({
  CandidateList: ({
    ...
    <div data-testid="candidate-list">
      {candidates.map((c: GenerationCandidateViewModel) => (
        <div key={c.candidateId} data-testid={`candidate-${c.candidateId}`}>
          <button data-testid={`accept-${c.candidateId}`} onClick={() => onAccept(c.candidateId)}>
            Accept
          </button>
          <button data-testid={`edit-${c.candidateId}`} onClick={() => onEdit(c.candidateId)}>
            Edit
          </button>
          <button data-testid={`reject-${c.candidateId}`} onClick={() => onReject(c.candidateId)}>
            Reject
          </button>
        </div>
      ))}
      <button data-testid="bulk-accept" onClick={() => onBulkAccept(["id1", "id2"])}>
        Accept All
      </button>
    </div>
  ),
...
vi.mock("../EditFlashcardModal", () => ({
  EditFlashcardModal: ({
    ...
    onSave: (candidate: GenerationCandidateViewModel) => void;
  }) =>
    isOpen && (
      <div data-testid="edit-modal">
        <button
          ...
          onClick={() => {
            if (candidate) {
              onSave({ ...candidate, front: "Updated Front", back: "Updated Back" });
            }
          }}
        >
          Save
        </button>
        <button data-testid="close-edit-modal" onClick={() => onOpenChange(false)}>
          Close
        </button>
      </div>
    ),
  ...
  ConfirmActionDialog: ({
    ...
    isOpen && (
      <div data-testid="confirm-dialog">
        <button data-testid="confirm-reject" onClick={onConfirm}>
          Confirm Reject
        </button>
        <button data-testid="cancel-reject" onClick={() => onOpenChange(false)}>
          Cancel
        </button>
      </div>
    ),
...
describe("GenerateView Component", () => {
  ...
  it("calls handleEdit with updated candidate when saving edit", async () => {
    ...
    expect(mockUseGeneration.handleEdit).toHaveBeenCalledWith(
  ...
  it("handles loading and success states for bulk accept", async () => {
    const { toast } = await import("sonner");
    mockUseGeneration.handleAccept.mockResolvedValueOnce(undefined).mockRejectedValueOnce(new Error("Failed"));

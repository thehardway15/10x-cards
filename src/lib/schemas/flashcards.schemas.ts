...
export const listFlashcardsQuerySchema = z.object({
  page: z.coerce.number().int().min(1).optional().default(1).describe("Page number (min: 1)"),
  pageSize: z.coerce.number().int().min(1).max(100).optional().default(20).describe("Items per page (max: 100)"),
  sortBy: z.enum(["createdAt", "front", "back"]).optional().default("createdAt").describe("Field to sort by"),

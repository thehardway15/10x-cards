...
export function useGeneration() {
  ...
  const handleAccept = async (candidateId: string) => {
    ...
    setState((prev) => ({
      ...prev,
      candidates: prev.candidates.map((c) => (c.candidateId === candidateId ? { ...c, status: "saving" } : c)),
    }));
    ...
    try {
      ...
      setState((prev) => ({
        ...prev,
        candidates: prev.candidates.map((c) => (c.candidateId === candidateId ? { ...c, status: "idle" } : c)),
